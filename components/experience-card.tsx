  ...
  return (
    <div className="space-y-4 pb-6 border-b border-border last:border-0 last:pb-0">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2">
        ...
        <div className="text-xs text-muted-foreground bg-surface-alt px-2 py-1 sm:px-3 sm:py-1 self-start mt-1 sm:mt-0 sm:self-auto">
          {period}
        </div>
      </div>
        ...
        <ul className="space-y-2">
          {achievements.map((achievement, index) => (
            <li key={index} className="flex text-sm text-zinc-300">
              <CheckCircle2 className="w-4 h-4 mr-2 text-zinc-500 flex-shrink-0 mt-0.5" />
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {technologies.map((tech, index) => (
            <Badge key={index} variant="outline" className="text-xs bg-surface-alt hover:bg-zinc-700">
              {tech}
            </Badge>
          ))}
        </div>
